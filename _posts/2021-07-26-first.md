...
With Taproot activation coming soon, now is a good time to look at [BIP-118](https://github.com/bitcoin/bips/blob/master/bip-0118.mediawiki) now known as Anyprevout and previously as Sighash_Noinput. This soft fork enables a powerful new mechanism for replacing presigned transactions in off-chain protocols. The new Lightning Network state update protocol [eltoo](https://blockstream.com/eltoo.pdf) was designed around the existence of Anyprevout.

In this blog post I will describe how to use the Bitcoin Core functional test framework to demonstrate a set of transactions needed for a simple implementation of the eltoo protocol. These tests use a fork of Bitcoin with an initial implementation of BIP-118 and Taproot. I also summarize the basic motivations and relevant features of Anyprevout, eltoo and Taproot to give context for understanding these test transactions.

The target audience for this write-up is anyone who wants to understand BIP-118, eltoo and Taproot better at the transaction and script level. You should also be able to build Bitcoin Core from source and understand Python scripts. Following the steps below you will be able to see eltoo transactions confirmed on a local regtest network and increaes your knowledge of how both eltoo, BIP-118 and Taproot work. My hope is that this will help more people understand how these technologies work and be capable of reviewing BIP-118. More people testing BIP-118 and implementing eltoo will help find problems and improvements for both. More review and testing is critical before BIP-118 can be adopted by the Bitcoin network and eltoo by the Lightning network.

# Tests

I recommend that you confirm your system is setup to build Bitcoin Core and run functional tests. [Jon Atack](https://twitter.com/jonatack) has a good summary [here](https://jonatack.github.io/articles/how-to-compile-bitcoin-core-and-run-the-tests).

Once you have confirmed your ability to run the functional tests (Step 8. in Jon's summary), you can follow the steps again, but instead of cloning from the Bitcoin Core repository, clone my fork and the eltoo-anyprevout branch:

```
clone --single-branch --branch eltoo-anyprevout https://github.com/remyers/bitcoin.git
```

Now you can run the new eltoo functional tests:

```
test/functional/simulate-eltoo.py
```

The resulting output should should that all tests passed:
...
The file /test/functional/simulate-eltoo.py runs two tests. The first is [test_bitcoinops_workshop_tapscript](https://github.com/remyers/bitcoin/blob/37a7490dc3b2128c0f7e34a463531f1123682d42/test/functional/simulate_eltoo.py#L1526) which goes through a short Taproot test based on the [Schnorr Taproot Tutorial](https://bitcoinops.org/en/schorr-taproot-workshop/#taproot) created by [Bitcoin Optech](https://bitcoinops.org/). I highly recommend these tutorials, but note that some changes in the Taproot BIP occurred after the workshop - specifically the change to "x only" public keys. You can also check out their recent [Preparing for Taproot](https://bitcoinops.org/en/preparing-for-taproot/) series.

### eltoo

You should first read [BIP-118](https://github.com/bitcoin/bips/blob/master/bip-0118.mediawiki) specificaton and the [eltoo](https://blockstream.com/eltoo.pdf) paper to understand these tests. The [Anyprevout][], [eltoo][] and [Taproot][] sections below also summarize details relevant to understanding these eltoo transaction tests.
...
All of the transactions before the [divider line](https://github.com/remyers/bitcoin/blob/37a7490dc3b2128c0f7e34a463531f1123682d42/test/functional/simulate_eltoo.py#L1694) are presigned transactions that in eltoo would occur without being immediately confirmed. Note that I am using a normal Schnorr key and not a 2 of 2 Multisig created with something like MuSig for these tests; except for how the public key and signatures are generated there should be no functional difference.

The next section uses the function ```test_transaction``` to test if a transaction is valid **without*** actually submitting it to the mempool. The first test checks if the ```update1_tx``` transaction is valid. We also test that ```settle1_tx``` signed with APO can not be rebound to ```update0_tx``` but that it can be if incorrectly signed with APOAS. The script then commits the ```update1_tx``` and generates CSV_DELAY number of blocks so that ```settle1_tx``` will be valid.

The next section starts by modifying ```settle1_tx``` and ```update2_tx``` to rebind them to the ```update1_tx``` that has now been confirmed on the blockchain. Because these transactions were signed with the APOAS sighash they do not commit to the transaction ID of a specific transaction. After these transactions are tested with ```test_transaction``` the script commits to one of them, the ```update2_tx``` to advance the channel to state 2.

We then create the ```update1b_tx``` transaction which attempts to spend the ```update2_tx``` output back to channel state 1. This transaction uses the same pregenerated signature used by ```update_1``` but rebinds it to ```update2_tx``` instead of ```update0_tx```. When ```update1b_tx``` is tested, it fails with the error message, ```'non-mandatory-script-verify-flag (Locktime requirement not satisfied)'``` because the signature commits to a locktime less than the locktime encoded in the spending script of ```update2_tx``` and so the script fails.

The next lines successfully commit to ```settle2_tx``` which exclusively spends from ```update_2``` because it uses the APO and not APOAS sighash. The final tests demonstrate how to spend the HTLCs using either the timeout via ```htlc_refund_tx``` or the hash preimage via ```htlc_claim_tx```. 

Note that because the HTLC refund and claim transactions are only signed by the channel partner collecting the HTLC, they are not presigned and can include an additional fee input and change output. They also do not need to use an Anyprevout sighash. The Update transactions have a single input and output, so by using SIGHASH_SINGLE along with APOAS a transaction fee input and change output can be added before commiting the transaction to the blockchain. Only the Settle transactions need to allocate part of the channel balance as transaction fees.
